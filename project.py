...
#   made by             : Debabrata Paul, Sagar Sinha and Abhay Kumar
#   session             : 2020-21
#   roll  no            : 
...
school_email = 'office@sfsnarengi.ac.in'
school_phone ='0361-2649856'
...
def add_student():
    conn = mysql.connector.connect(host='localhost',database='report_card',user='root', password='MysqLDeV123',autocommit=True )
    cursor = conn.cursor()
    clear()
...
def add_marks():
    ...
    sql = 'insert into marks(admno,term,session,phy,chem,math,eng,comp) values(' +admno+',"'+term+'","'+session+'",'+phy+','+chem+','+math+','+eng+','+comp+');'
    cursor.execute(sql)
    conn.close()
...
def report_whole_session():
    ...
    print('-'*120)
    t = PrettyTable(['admno', 'Name', 'Father Name','Class','section','Term', 'Phy', 'Chem', 'Math', 'Eng', 'Comp', 'Total'])
